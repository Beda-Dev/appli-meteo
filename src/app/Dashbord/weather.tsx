...
export default function Weather({ ville }: props) {
  ...
  const [tempsactuel, setTempsActuel] = useState<{
    hours: number;
    minutes: number;
  }>({ hours: 0, minutes: 0 });
  const [synchro, setSynchro] = useState<string>("");

  const mettreAJourStockage = (
    nomVille: string,
  ...
  useEffect(() => {
    try {
      fetch(
        `${API.url}weather?q=${ville}&units=metric&lang=fr&APPID=${API.clé}`
      )
        .then((res) => res.json())
        .then((result) => {
          setResultat(result);
  ...
  return (
    <div className=" text-white rounded-xl bg-white bg-opacity-10 backdrop-blur-2xl w-[275px] h-[250px] p-2">
      <h1 className="pl-3 mt-0 text-2xl italic">Météo actuelle </h1>

      <div className="pb-1">
        <h3 className="pl-2 text-sm font-sans font-light">
          {resultat?.cod == 200 && tempsactuel ? (
            <p>
              {tempsactuel.hours + ConversionHeureSeconde(resultat.timezone)}h :{" "}
              {tempsactuel.minutes}min
            </p>
          ) : resultat?.cod === "404" ? (
            <p>Aucun resultat</p>
          ) : (
        ...
        {resultat && resultat.cod === 200 ? (
            ...
            <div className="flex">
              ...
              <div className="max-w-full max-h-full text-left flex flex-col justify-start">
                ...
                <h1 className="text-5xl font-mono text-center">
                  {Math.round(resultat?.main.temp)}°C
                </h1>
                <h2 className="text-center">
                  {resultat?.weather[0].description}
                </h2>
              </div>
            </div>
            ...
            <div className="flex ">
              ...
              <div className="w-1/4 text-center flex flex-col items-center">
                <Image
                  ...
                  className=""
                />
                <p className="text-sm text-center">{resultat?.wind.speed} km</p>
              </div>
              <div className="w-1/4 text-center flex flex-col items-center">
                <Image
                  src="/visibility.png"
          ...
          <div className="text-center">
            <p>
              Opps !! ville non trouvée , essayez avec une autre orthographe
            </p>
          </div>
        ) : (
          <div className="text-center">
